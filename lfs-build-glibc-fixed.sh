#!/bin/bash
# Auto-LFS-Builder - Direct LFS Build Script (FIXED)
...
setup_lfs_environment() {
    ...
    # CRITICAL FIX: Enhanced MB_LEN_MAX configuration
    mkdir -p "$LFS/usr/share"
    cat > "$LFS/usr/share/config.site" << 'EOF'
# Enhanced config.site for glibc compatibility
ac_cv_sys_mb_len_max=16
ac_cv_explicit_max_align=yes
libc_cv_mb_len_max=16
libc_cv_explicit_max_align=yes
EOF
    export CONFIG_SITE="$LFS/usr/share/config.site"
    
    # Create critical header files to prevent glibc issues
    mkdir -p "$LFS/usr/include/bits"
...
EOF
    ...
    # Set up enhanced environment variables for glibc compatibility
    export LC_ALL=POSIX
    export LFS_TGT=$(uname -m)-lfs-linux-gnu
    export PATH="$LFS/tools/bin:/bin:/usr/bin"
    export MAKEFLAGS="-j$PARALLEL_JOBS"
    
    # CRITICAL: Enhanced CFLAGS for glibc compatibility
    export CFLAGS="-D_MB_LEN_MAX=16 -D__MB_LEN_MAX=16 -DMULTIBYTE -g -O2"
    export CXXFLAGS="-D_MB_LEN_MAX=16 -D__MB_LEN_MAX=16 -DMULTIBYTE -g -O2"
    export CPPFLAGS="-D_MB_LEN_MAX=16 -D__MB_LEN_MAX=16 -DMULTIBYTE"
    
    # Additional glibc-specific environment
    export libc_cv_slibdir=/usr/lib
    export libc_cv_rtlddir=/usr/lib
    export libc_cv_cpp_explicit_max_align=yes
    export libc_cv_mb_len_max=16
...
# Build Glibc with enhanced fixes
build_glibc() {
    log_phase "Building Glibc (Enhanced Fix)"
    ...
    # CRITICAL FIX: Apply multiple patches for gcc-macros.h issue
    
    # Fix 1: Create a pre-configuration limits.h
...
EOF
    ...
    # Fix 2: Create explicit MB_LEN_MAX definition for configure
    export ac_cv_sys_mb_len_max=16
    export libc_cv_mb_len_max=16
    export libc_cv_explicit_max_align=yes
    
    # Fix 3: Enhanced CFLAGS for this specific build
    export GLIBC_CFLAGS="-D_MB_LEN_MAX=16 -D__MB_LEN_MAX=16 -DMULTIBYTE -g -O2"
    export GLIBC_CXXFLAGS="-D_MB_LEN_MAX=16 -D__MB_LEN_MAX=16 -DMULTIBYTE -g -O2"
    
    mkdir -v build
    cd build
    
    # Enhanced configparms for glibc
    cat > configparms << 'EOF'
...
EOF
    ...
    # CRITICAL: Use cache variables to force correct detection
    ../configure \
        --prefix=/usr \
        --host="$LFS_TGT" \
    ...
    # CRITICAL: Build with enhanced environment
    log_info "Starting glibc build with MB_LEN_MAX fixes"
    CFLAGS="$GLIBC_CFLAGS" CXXFLAGS="$GLIBC_CXXFLAGS" make_build
    
    # Install with proper environment
    CFLAGS="$GLIBC_CFLAGS" CXXFLAGS="$GLIBC_CXXFLAGS" make DESTDIR="$LFS" install
    ...
    log_success "Glibc built and installed with enhanced fixes"
}
...
main() {
    ...
    log_phase "Starting Auto-LFS-Builder (FIXED VERSION)"
    ...
    log_warning "USING ENHANCED GLIBC FIXES FOR gcc-macros.h ISSUE"
    echo
    ...
    echo
    log_success "GLIBC BUILD FIXES SUCCESSFULLY APPLIED!"
}
