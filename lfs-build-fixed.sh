#!/bin/bash
# Auto-LFS-Builder - Enhanced Version with Glibc Fix
# SPDX-License-Identifier: MIT
# Copyright (c) 2025 The LFS Automation Team

# Modified error handling to prevent cleanup loops
set -euo pipefail
...
# Default configuration - Fixed workspace path
...
# Enhanced make build function with better error handling
make_build() {
    local start_time=$(date +%s)
    echo "[BUILD] Starting make with arguments: $*"
    
    # Check if we're in a valid build directory
    if [[ ! -f Makefile && ! -f makefile ]]; then
        log_error "No Makefile found in current directory: $(pwd)"
        return 1
    fi
    
    if [[ "$VERBOSE" == "true" ]]; then
        MAKEFLAGS="${MAKEFLAGS} V=1 VERBOSE=1" \
        timeout 7200 make -j"$PARALLEL_JOBS" --debug=v "$@" 2>&1 | tee -a "$LOG_PATH"
        local exit_code=${PIPESTATUS[0]}
    else
        timeout 7200 make -j"$PARALLEL_JOBS" "$@"
        local exit_code=$?
    fi
    ...
    if [[ $exit_code -eq 124 ]]; then
        log_error "Build timed out after 2 hours"
        return 1
...
build_kernel_headers() {
    ...
    run_verbose make headers || return 1
    find usr/include -type f ! -name '*.h' -delete
    cp -rv usr/include "$LFS/usr" || return 1
...
# Enhanced Glibc build with better error handling
build_glibc() {
    ...
    if [[ ! -d "$LFS/usr/include/linux" ]]; then
        log_error "Linux kernel headers not found. Run build_kernel_headers first."
        return 1
    fi
    
    if [[ ! -d "$LFS/tools" ]]; then
        log_error "Cross-compilation tools not found. Run build_cross_tools first."
        return 1
    fi
    ...
    case $(uname -m) in
        i?86)   echo "CFLAGS += -march=i686" > configparms ;;
        x86_64) echo "CFLAGS += -mtune=native" > configparms ;;
    esac
    echo "rootsbindir=/usr/sbin" >> configparms
    
    log_info "Configuring Glibc..."
    
    # Set additional environment variables for glibc
    ...
    ../configure \
        ...
        log_error "Glibc configure failed"
        if [[ -f config.log ]]; then
            log_info "Last 50 lines of config.log:"
            tail -50 config.log
        fi
        return 1
    }
    ...
    if ! make DESTDIR="$LFS" install; then
    ...
    sed '/RTLDLIST=/s@/usr@@g' -i "$LFS/usr/bin/ldd" || {
        log_warning "Failed to fix ldd symlink"
    }
...
build_core_tools() {
    ...
    for tool in "${tools[@]}"; do
        ...
        tar -xf "$tool" || return 1
        cd "$name-$version"
        
        case "$name" in
            "bash")
                ./configure --prefix=/usr --host="$LFS_TGT" --without-bash-malloc || return 1
                make_build || return 1
                make DESTDIR="$LFS" install || return 1
                ;;
            "coreutils")
                ./configure --prefix=/usr --host="$LFS_TGT" --enable-install-program=hostname || return 1
                make_build || return 1
                make DESTDIR="$LFS" install || return 1
                ;;
            *)
                ./configure --prefix=/usr --host="$LFS_TGT" || return 1
                make_build || return 1
                make DESTDIR="$LFS" install || return 1
                ;;
        esac
...
main() {
    ...
    log_phase "Starting Auto-LFS-Builder (Enhanced)"
    ...
    # Build steps with individual error checking
    ...
    if ! setup_lfs_environment; then
        log_error "LFS environment setup failed"
        return 1
    fi
    ...
    if ! build_cross_tools; then
        log_error "Cross-tools build failed"
        return 1
    fi
    ...
    # Debug info before glibc build
    log_info "=== Pre-Glibc Debug Info ==="
    log_info "LFS: $LFS"
    log_info "LFS_TGT: $LFS_TGT" 
    log_info "CONFIG_SITE: $CONFIG_SITE"
    log_info "Tools directory: $(ls -la "$LFS/tools" 2>/dev/null | wc -l) items"
    log_info "Headers directory: $(ls -la "$LFS/usr/include" 2>/dev/null | wc -l) items"
    log_info "============================"
    ...
    if ! build_core_tools; then
        log_error "Core tools build failed"
        return 1
    fi
    ...
    if ! install_networking; then
        log_error "Network setup failed"
        return 1
    fi
    
    if ! install_gnome; then
        log_error "GNOME setup failed"
        return 1
    fi
    
    if ! create_boot_config; then
        log_error "Boot config failed"
        return 1
    fi
    
    if ! finalize_system; then
        log_error "System finalization failed"
        return 1
    fi
    
    if ! create_system_image; then
        log_error "System image creation failed"
        return 1
    fi
